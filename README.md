# EventPlannerTasksAI MCP Server

**200+ AI-powered administrative workflows for Event planners, wedding coordinators, and corporate event managers.**
...
10. Ask the user to approve with their EventPlannerTasksAI account email; use manual license-key entry only as fallback.
...
EventPlannerTasksAI gives your AI assistant product-specific workflows for Event planners, wedding coordinators, and corporate event managers.

Example tasks:

- Write an event proposal
- Draft a vendor contract
- Prepare a run-of-show document
- Create a post-event client summary
- Write a vendor inquiry email

**Privacy model:** EventPlannerTasksAI servers handle authentication, credits, catalog/search metadata, and licensed skill delivery. EventPlannerTasksAI does not process the user's task content. The user's chosen AI assistant or LLM performs the task work according to that provider's privacy terms. If you use a cloud AI assistant, your prompts or documents may be sent to that AI provider; they are not processed by EventPlannerTasksAI.
...
| Tool | Description |
|------|-------------|
| `eventplannertasksai_search` | Search available workflows |
| `eventplannertasksai_execute` | Get the full expert framework for a workflow, after confirmation |
| `eventplannertasksai_balance` | Check your remaining credit balance |
| `eventplannertasksai_categories` | Browse workflows by category |

---

## Support

- **Email:** support@eventplannertasksai.com
- **Website:** [eventplannertasksai.com](https://eventplannertasksai.com)
- **Getting Started:** [eventplannertasksai.com/getting-started.html](https://eventplannertasksai.com/getting-started.html)
